...
class Motor {
	public:
		...
		int current_count; //current number of encoder pulses
		int previous_count; //previous number of encoder pulses before update
		...
		float kv; //Velcoity constant for cascade control

		//Methods
		void change_control_constants(float kpos, float kvel); //allows user to change the constants used in CascadeControl
		...
		void calc_velocity(double Ts); //calculates velocoity of motor based on encoder counts, and counts per rotation of motor
		void Tune_Velocity_Loop(float kvel); //allows user to alter kv to find the brink of instability 
		void Tune_Position_Loop(float kpos, float kvel, float current_position, float desired_position); //allows user to alter kp to find overshoot (very similar to CascadeControl)
