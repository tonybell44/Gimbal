...
//Get all acceleration values in an array (make sure static or global input)
//Make sure the order for 0,1,2 in the input array is x,y,z respectively
void MPU6050::get_all_accel(float accel[3])
{
	accel[0] = (_ax/accel_sensitivity) - _ax_bias;
	accel[1] = (_ay/accel_sensitivity) - _ay_bias;
	accel[2] = (_az/accel_sensitivity) - _az_bias;
}
...
//Get all angular velocity values in an array (make sure static or global input)
//Make sure the order for 0,1,2 in the input array is x,y,z respectively
void MPU6050::get_all_ang_vel(float gyro[3])
{
	gyro[0] = (_wx/gyro_sensitivity) - _wx_bias;
	gyro[1] = (_wy/gyro_sensitivity) - _wy_bias;
	gyro[2] = (_wz/gyro_sensitivity) - _wz_bias;
}
