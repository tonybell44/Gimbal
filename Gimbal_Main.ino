...
unsigned long prev;
double Ts = 0.001;                     //Ts is in seconds; time between every new state (for cascade control, we are going to use this to control the velocity loop)

//Global Variables for Motors
Motor roll_Motor = Motor(32,36,7,12,1,2096,4); 

//Setup and initialize all neccessary modules/variables
void setup() {
  Serial.begin(9600);
  roll_Motor.motor_setup();
  roll_Motor.change_control_constants(2.5, 1.8);
  imu.initialize();
